...
export class Enemy extends Actor {
    ...
    private moveToNextNode(game: Game, deltaTime: number) {
        ...
        if (Math.abs(distanceVector.x) + Math.abs(distanceVector.y) < 1) {
            ...
            if (
                this.targetNodeIdx! >
                game.gameMap!.mapNodes!.nodes.length - 1
            ) {
                this.onMapExit(game);
            ...
            this.targetNode =
                game.gameMap!.mapNodes!.nodes[this.targetNodeIdx!];
        }
    }
    ...
    private onCollide(
        ...
        console.log(
            `hit for  ${damage}, health: ${this.health}/${this.maxHealth}`
        );
