	...
	override async init(game: Game) {
		...
		game.stage.on('mouseup', async () => {
			...
			if (this.isDragging) {
				...
				const sampledOverlayPixel = game.gameMap!.getOverlayPixel(this.ghost!.x, this.ghost!.y);
				...
				if (isSame) {
					const tower = await this.spawnTower(game, game.controls.mouseX, game.controls.mouseY);
...
export class GameUI extends Actor {
	...
	private async setTowers(game: Game) {
		const spawnGatlingGunner = async (game: Game, posX: number, posY: number) => {
