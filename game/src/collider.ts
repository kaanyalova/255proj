...
export class Collider {
    ...
    collidesWithOther(other: Collider): boolean {
        return this.shapes.some((thisCollider) =>
            ...
            other.shapes.some((otherCollider) =>
                thisCollider.intersects(otherCollider)
            )
        );
    }
    ...
    static rayCast(self: Rectangle, other: Rectangle, ray: Vector2): boolean {
        ...
        return (
            topIntersect || bottomIntersect || leftIntersect || rightIntersect
        );
    }
    ...
    private static doesLineIntersect(
        ...
        return (
            this.ccw(x1, x2, y2) !== this.ccw(y1, x2, y2) &&
            this.ccw(x1, y1, x2) !== this.ccw(x1, y1, y2)
        );
    }
